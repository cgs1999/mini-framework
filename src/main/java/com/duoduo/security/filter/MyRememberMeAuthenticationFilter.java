 ...
 * a remember-me authentication token if a {@link org.springframework.security.web.authentication.RememberMeServices}
 * implementation so requests.
 * <p>
 * Concrete <code>RememberMeServices</code> implementations will have their
 * {@link org.springframework.security.web.authentication.RememberMeServices#autoLogin(HttpServletRequest, HttpServletResponse)}
 * method called by this filter. The <code>Authentication</code> or <code>null</code> returned by that method will be
 * placed into the <code>SecurityContext</code>. The <code>AuthenticationManager</code> will be used, so that any
 * concurrent session management or other authentication-specific behaviour can be achieved. This is the same pattern as
 * with other authentication mechanisms, which call the <code>AuthenticationManager</code> as part of their contract.
 * </p>
 * <p>
 * If authentication is successful, an
 * {@link org.springframework.security.authentication.event.InteractiveAuthenticationSuccessEvent} will be published to
 * the application context. No events will be published if authentication was unsuccessful, because this would generally
 * be recorded via an <code>AuthenticationManager</code>-specific application event.
 * </p>
 * @author Ben Alex
 */
public class MyRememberMeAuthenticationFilter extends GenericFilterBean implements ApplicationEventPublisherAware {
	 ...
	 * <tt>RememberMeServices</tt> <tt>autoLogin</tt> method. This method will not be called when no remember-me token
	 * is present in the request and <tt>autoLogin</tt> returns null.
	 */
